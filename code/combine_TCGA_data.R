...
#读取RNA的sample sheet
RNA_metadata=read.table("RNAseq_sample_sheet.tsv",header=T,sep="\t")
#替换.为下划线，转换成小写
names(RNA_metadata)=tolower(gsub("\\.","_",names(RNA_metadata)))
#调用merge_TCGA函数合并RNA的表达矩阵
rnaCounts=merge_TCGA(metadata=RNA_metadata, path="RNAseq_data", data.type="RNAseq")
#保存RNA表达矩阵
write.table(file="combined_RNA_Expr.txt",rnaCounts,sep="\t",quote=F)
